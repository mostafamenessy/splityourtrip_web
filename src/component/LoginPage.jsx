...
function LoginPage() {
    ...
    const { setIsUser, setUserCurrency, setLoginModal, setLoginData, registerModal, setRegisterModal, baseUrl } = useContextex();

    useEffect(() => {
        const fetchSmsTypeDataAsync = async () => {
            try {
                const response = await axios.post(`${baseUrl}user_api/sms_type.php?`, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                });
                if (response?.data?.ResponseCode === '200') {
                    setOtpType(response?.data?.SMS_TYPE);
                    setisAuth(response?.data?.otp_auth);
                }
            } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        const mobileNumber = FormattedNumber.split(" ").splice(1).join("");

        try {

            const response = await axios.post(
                `${baseUrl}user_api/u_login_user.php?`,
                { mobile: mobileNumber, ccode: countryCallingCode, password },
                { headers: { 'Content-Type': 'application/json' } }
            );

            const data = response?.data;

            const toastId = data?.ResponseCode === '200' ? "success" : "error";
            showToast({ title: data?.ResponseMsg, id: toastId });
        ...
        <>
            <div className="py-[10px] px-[15px]">
                ...
                <div className="mt-[15px]">
                    ...
                    <div style={{ border: errors.password ? "1px solid red" : "1px solid #80808038" }} className="relative mt-[10px] flex items-center gap-[10px] py-[15px] px-[18px] rounded-[12px] mb-[5px]">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <input type={passtype} onChange={(e) => setPassword(e.target.value)} className='w-[90%] outline-none text-[15px]' placeholder='Password' />
                    ...
                    <div className="flex items-center justify-between mt-[10px]">
                        ...
                        <button
                            type="button"
                            className="text-[14px] text-rose-400"
                ...
                <button onClick={handleSubmit} className="bg-[#2D71FE] hover:bg-[#2d73fec9] w-[100%] text-[17px] py-[12px] text-white font-[500] rounded-[10px] mt-[20px]">{t("Log In")}</button>
                <p onClick={() => setRegisterModal(true)} className='mt-[10px] font-bold text-[16px] cursor-pointer text-center'>Not a member? <span className='text-[#2D71FE]'>Register here</span></p>

            </div>

            {registerModal && <div onClick={() => setRegisterModal(false)} className="bottem_sheet items-center">
                <div onClick={(e) => e.stopPropagation()} className="login_model">
                    <RegistrationPage otpType={otpType} isAuth={isAuth} />
                </div>
            </div>}

            {showForgetPassMd && <div onClick={() => setShowForgetPassMd(false)} className="bottem_sheet items-center">
                <div onClick={(e) => e.stopPropagation()} className="login_model">
                    <ForgotPass otpType={otpType} isAuth={isAuth} setShowForgetPassMd={setShowForgetPassMd} setLoginModal={setLoginModal} />
                </div>
            </div>}
